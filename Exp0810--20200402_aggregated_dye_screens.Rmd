...
Exp0468_prot_names <- read_csv("../old_dye_screens_Exp0405_to_Exp0508/Exp0468--20181205_eIF2B_with_ISRIB/intermediate/Exp0468_protein_smoothed.csv") %>% names()
Exp0468_prot_names
paths_processed_468 <- "../old_dye_screens_Exp0405_to_Exp0508/Exp0468--20181205_eIF2B_with_ISRIB/intermediate/"

Exp0468_dye_to_well <- dir(paths_processed_468, full.names = T, pattern = "topdye_to_well_mapping_confirmation.csv") %>% dye_to_layout( . )

format_half_screen(paths_processed_468,
                   "protein",
                   "Exp0468",
                   Exp0468_dye_to_well
                   )

# eIF2B and light chain screens were both 
df_468_noISRIB <- read_csv("../old_dye_screens_Exp0405_to_Exp0508/Exp0468--20181205_eIF2B_with_ISRIB/intermediate/Exp0468_protein_smoothed.csv") %>% 
                  pivot_longer(-Temperature, names_to = "raw_var", values_to = "value")  %>%
  mutate(Temperature = .$Temperature %>% as.numeric()) %>%
                left_join(., dye_to_well, by = "raw_var") %>%
                select( -raw_var )%>%
...
df_468_noISRIB
dir(paths_processed_468, full.names = T, pattern = "dye_to_well_mapping_confirmation.csv")
```
```{r}
